...
CLAUDE_SONNET_3_5_MODEL_ID = "eu.anthropic.claude-3-5-sonnet-20240620-v1:0"  # Standard model ID format
...
SYSTEM_PROMPT_CROSS_ANALYSIS_HE = (
    ...
    "המטרה היא לזהות מאפיינים משותפים, דפוסים, או קשרים בין סוגי ההמלצות הטיפוליות לבין מאפייני המקרים (כגון סוג המחלה, שלב משוער, או מאפיינים קליניים אחרים שניתן להסיק מהמידע שנמסר). "
    ...
    "אנא הצג את הממצאים שלך בצורה מובנית וברורה, רצוי לקבץ אותם לפי סוג ההמלצה העיקרי, לפי סוג המחלה, או לפי מאפיינים משותפים משמעותיים שזיהית. "
...
אנא נתח מקרים אלו כדי למצוא מאפיינים משותפים בין המטופלים או המחלות, בהתבסס על סוגי ההמלצות שניתנו ותיאורי המחלה.
    ...
    try:
        with open(INPUT_CSV_PATH, 'r', encoding='utf-8') as infile:
            ...
            if reader.fieldnames is None:
                ...
                if not (CSV_FIELD_DISEASE in reader.fieldnames and CSV_FIELD_RECOMMENDATIONS in reader.fieldnames):
                    print(
                        f"ERROR: Essential columns ('{CSV_FIELD_DISEASE}', '{CSV_FIELD_RECOMMENDATIONS}') not found in CSV headers. Exiting."
                    )
                    sys.exit(1)
            ...
            for i, row in enumerate(reader):
                ...
                if not disease_info or not recommendations_info:
                    print(f"Warning: Skipping record {i+1} due to missing disease or recommendations.")
                    continue

                cases_data.append({"disease": disease_info, "recommendations": recommendations_info})
