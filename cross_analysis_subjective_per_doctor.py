...
SYSTEM_PROMPT_CROSS_ANALYSIS_HE = (
    "כאונקולוג מומחה, משימתך היא לנתח את החלטות הטיפול של רופא ספציפי על סמך מקרי מטופלים שיוצגו בפניך. המטרה המרכזית היא לזהות דפוסים כלליים, הטיות והתנהגויות קבועות בקבלת החלטות של הרופא."
    "בצע ניתוח פתוח ובלתי-מוגבל של הנתונים, זהה מגמות, הטיות והבדלים בטיפול ללא הגבלה לדוגמאות מוגדרות מראש. חפש קשרים, דפוסים ומאפיינים ייחודיים שמאפיינים את סגנון הטיפול של הרופא."
    "נתח את הנתונים בחירות ובלי הנחות מוקדמות - מה שהנתונים מגלים עליהם. זהה כל דבר מעניין, חריג או מובהק בהתנהגות הרפואית של הרופא."
    "הצג את ממצאיך בצורה מפורטת ומנומקת, תוך הסתמכות על הנתונים שסופקו בלבד. בסס את הניתוח שלך על הידע הרפואי העדכני ביותר."
    "כתוב את הניתוח שלך בעברית בלבד."
)
...
def main():
    ...
    for doctor_name, doctor_cases_df in grouped_by_doctor:
        ...
        raw_data_filename = os.path.join(OUTPUT_RAW_DATA_FOLDER, f"{str(doctor_name).replace(' ', '_')}_raw_data.csv")
        try:
